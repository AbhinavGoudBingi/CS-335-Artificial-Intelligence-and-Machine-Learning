...
class FullyConnectedLayer:
    ...
    def backwardpass(self, lr, activation_prev, delta):
        ...
        if self.activation == 'relu':
            grad = gradient_relu_of_X(self.data,delta)
            ret = grad@((self.weights).T)
            self.weights = self.weights - lr*((activation_prev.T)@grad)/len(activation_prev)
            self.biases = self.biases - lr*(np.sum(grad,axis=0,keepdims=True))/len(activation_prev)
            return ret
            # raise NotImplementedError
        elif self.activation == 'softmax':
            grad = gradient_softmax_of_X(self.data,delta)
            ret = grad@((self.weights).T)
            self.weights = self.weights - lr*((activation_prev.T)@grad)/len(activation_prev)
            self.biases = self.biases - lr*(np.sum(grad,axis=0,keepdims=True))/len(activation_prev)
            return ret
...
class ConvolutionLayer:
    ...
    def forwardpass(self, X):
        ...
        # TODO
        if self.activation == 'relu':
            out_shape = (X.shape[0],self.out_depth,self.out_row,self.out_col)
            out = np.ones(out_shape)
            for i in range(self.out_row):
                for j in range(self.out_col):
                    out[:,:,i,j]=np.sum(X[:,np.newaxis,:,i*self.stride:min(X.shape[2],i*self.stride+self.filter_row),j*self.stride:min(X.shape[3],j*self.stride+self.filter_col)]*(self.weights[np.newaxis,:,:,:,:]),axis=(2,3,4))
            #raise NotImplementedError
            self.data = relu_of_X(out + self.biases[np.newaxis,:,np.newaxis,np.newaxis])
            return self.data
        else:
            print("ERROR: Incorrect activation specified: " + self.activation)
            exit()
    ...
    def backwardpass(self, lr, activation_prev, delta):
        ...
        if self.activation == 'relu':
          ...
          for i in range(self.out_row):
              for j in range(self.out_col):
                  out[:,:,i*self.stride:min(activation_prev.shape[2],i*self.stride+self.filter_row),j*self.stride:min(activation_prev.shape[3],j*self.stride+self.filter_col)]+=np.sum((self.weights[np.newaxis,:,:,:,:])*delta[:,:,np.newaxis,i:i+1,j:j+1],axis=1)
                  w_gradient = w_gradient + np.sum(activation_prev[:,np.newaxis,:,i*self.stride:min(activation_prev.shape[2],i*self.stride+self.filter_row),j*self.stride:min(activation_prev.shape[3],j*self.stride+self.filter_col)]*delta[:,:,np.newaxis,i:i+1,j:j+1],axis=0)

          self.weights = self.weights - lr*(w_gradient)/activation_prev.shape[0]
          self.biases = self.biases - lr*(np.sum(delta,axis=(0,2,3)))/activation_prev.shape[0]
          return out
        else:
            print("ERROR: Incorrect activation specified: " + self.activation)
            exit()
...
class AvgPoolingLayer:
    ...
    def forwardpass(self, X):
        ...
        # TODO
        out = np.ones((X.shape[0],self.out_depth,self.out_row,self.out_col))
        for i in range(self.out_row):
            for j in range(self.out_col):
                out[:,:,i,j]=np.sum(X[:,:,i*self.stride:min((i+1)*self.stride,X.shape[2]),j*self.stride:min((j+1)*self.stride,X.shape[3])],axis=(2,3))/(self.filter_row*self.filter_col)
        return out
        pass
        # END TODO
        ###############################################
    ...
    def backwardpass(self, alpha, activation_prev, delta):
        ...
        # TODO
        return np.repeat(np.repeat(delta,self.stride,axis=3),self.stride,axis=2)/(self.stride**2)
        pass
        # END TODO
        ###############################################
...
class MaxPoolingLayer:
    ...
    def forwardpass(self, X):
        ...
        for i in range(self.out_row):
            for j in range(self.out_col):
                out[:,:,i,j] = np.max(X[:,:,i*self.stride:min((i+1)*self.stride,X.shape[2]),j*self.stride:min((j+1)*self.stride,X.shape[3])],axis=(2,3))
        return out
        pass
        # END TODO
        ###############################################
    ...
    def backwardpass(self, alpha, activation_prev, delta):
        ...
        # TODO
        out = np.ones(delta.shape)
        for i in range(self.out_row):
            for j in range(self.out_col):
                out[:,:,i,j]=np.max(activation_prev[:,:,i*self.stride:min((i+1)*self.stride,activation_prev.shape[2]),j*self.stride:min((j+1)*self.stride,activation_prev.shape[3])],axis=(2,3))
        return (np.repeat(np.repeat(out,self.stride,axis=3),self.stride,axis=2)==activation_prev)*np.repeat(np.repeat(delta,self.stride,axis=3),self.stride,axis=2)
        pass
        # END TODO
        ###############################################
...
class FlattenLayer:
    ...
    def forwardpass(self, X):
        ...
        pass
    def backwardpass(self, lr, activation_prev, delta):
        return delta.reshape(self.n,self.depth,self.rows,self.columns)
        pass
        # END TODO
...
def gradient_softmax_of_X(X, delta):
    ...
    for i in range(X.shape[1]):
        ...
        temp[:,i] = X[:,i]
        temp1 = delta*(X*(X[:,i].reshape(X.shape[0],1)) - temp)
        out[:,i] = np.sum(-1*(temp1),axis=1)
    return out
    #raise NotImplementedError
    # END TODO
