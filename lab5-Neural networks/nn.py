...
class NeuralNetwork:
    ...
    def computeLoss(self, Y, predictions):
        ...
        #print(Y)
        N = Y.shape[0]
        p_copy = predictions[-1].copy()
        p_copy = (p_copy>0)*p_copy+(10**(-9))*(p_copy<=0)
        return -np.sum(np.sum(Y*(np.log(p_copy))))/N
    ...
    def feedforward(self, X):
        ...
        for layer in self.layers:
            out_values.append(layer.forwardpass(out_values[-1]))
        return out_values
        # raise NotImplementedError
        # END TODO
    ...
    def backpropagate(self, activations, Y):
        ...
        a_copy = (a_copy>0)*a_copy+(10**(-9))*(a_copy<=0)
        delta = -Y/a_copy
        l = len(self.layers)
        for i in range(l):
            delta = self.layers[l-i-1].backwardpass(self.alpha,activations[l-i-1],delta)
        #raise NotImplementedError
        # END TODO