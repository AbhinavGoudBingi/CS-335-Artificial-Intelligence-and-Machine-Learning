...
def split_data(X, Y, train_ratio=0.8):
    ...
    ## TODO
    n = X.shape[0]
    p = int(np.floor(n*train_ratio))
    X_train = X[None:p,None:None]
    Y_train = Y[None:p,None:None]
    X_test = X[p:None,None:None]
    Y_test = Y[p:None,None:None]
...
def preprocess(X, Y):
    ...
    for i in range(1,X.shape[1]):
        if str(X[0][i]).replace('.','',1).isdigit():
            z = (np.array(X[:,i], copy=False, subok=True, ndmin=2).T).astype(np.float)
            x = np.hstack((x,normalize(z)))
        else:
            z = np.array(X[:,i])
            labels = list(np.unique(z))
            Z = one_hot_encode(z,labels)
            x = np.hstack((x,Z))
    y = normalize(Y)
    return x, y
    ## END TODO
