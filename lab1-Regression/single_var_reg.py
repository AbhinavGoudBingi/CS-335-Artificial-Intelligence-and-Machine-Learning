...
def mse(X, Y, w, b):
    ...
    ## TODO
    n = X.shape[0]
    mse = np.sum(np.square((w*X+b)-Y))/(2*n)
    ## END TODO
...
def ordinary_least_squares(X_train, Y_train, X_test, Y_test, lr=0.001, max_iter=200):
    ...
    for i in range(max_iter):
        ...
        w = w - lr*(np.sum(np.multiply(X_train,(w*X_train+b)-Y_train)))/X_train.shape[0]
        b = b - lr*(np.sum((w*X_train+b)-Y_train))/X_train.shape[0]
